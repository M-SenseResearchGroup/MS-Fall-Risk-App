...
void b_strcmp(const char a_data[], const int a_size[2], boolean_T b_bool[2])
{
  ...
  if (0 <= loop_ub - 1) {
    memcpy(&t1_f1_data[0], &a_data[0], (unsigned int)(loop_ub * (int)sizeof(char)));
  }
  ...
  if (a_size[1] == 2) {
    ...
    do {
      exitg1 = 0;
      if (loop_ub < 2) {
        if (cv4[(unsigned char)t1_f1_data[loop_ub]] != cv4[(int)cv5[loop_ub]]) {
          exitg1 = 1;
        } else {
          loop_ub++;
        }
  ...
  if (a_size[1] == 5) {
    ...
    do {
      exitg1 = 0;
      if (loop_ub < 5) {
        if (cv4[(unsigned char)t1_f1_data[loop_ub]] != cv4[(int)cv6[loop_ub]]) {
          exitg1 = 1;
        } else {
          loop_ub++;
        }
