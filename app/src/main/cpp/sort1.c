...
void sort(emxArray_creal_T *x, int idx_data[], int idx_size[2])
{
  ...
  vlen = x->size[dim] - 1;
  x_idx_0 = (short)x->size[dim];
  i52 = vwork->size[0];
  vwork->size[0] = x_idx_0;
  emxEnsureCapacity_creal_T(vwork, i52);
  x_idx_0 = (short)x->size[0];
  x_idx_1 = (short)x->size[1];
  idx_size[0] = x_idx_0;
  idx_size[1] = x_idx_1;
  vstride = 1;
  for (k = 0; k < dim; k++) {
  ...
  for (i = 0; i < npages; i++) {
    ...
    for (j = 0; j < vstride; j++) {
      ...
      for (k = 0; k <= vlen; k++) {
        i52 = pageoffset + k * vstride;
        x->data[i52] = vwork->data[k];
        idx_data[i52] = iidx_data[k];
      }
    }
  }
