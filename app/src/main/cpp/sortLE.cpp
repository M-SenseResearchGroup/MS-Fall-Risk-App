...
boolean_T sortLE(const emxArray_creal_T *v, int idx1, int idx2)
{
  ...
  if (rtIsNaN(v->data[idx2 - 1].re) || rtIsNaN(v->data[idx2 - 1].im)) {
    ...
    if ((std::abs(v->data[idx1 - 1].re) > 8.9884656743115785E+307) || (std::abs
         (v->data[idx1 - 1].im) > 8.9884656743115785E+307)) {
      SCALEA = true;
    } else {
      SCALEA = false;
    }

    if ((std::abs(v->data[idx2 - 1].re) > 8.9884656743115785E+307) || (std::abs
         (v->data[idx2 - 1].im) > 8.9884656743115785E+307)) {
      SCALEB = true;
    } else {
      SCALEB = false;
    }
